...
from config import (
    TOKEN, GUILD_ID,
    WELCOME_CHANNEL_ID, LOG_CHANNEL_ID, AUTO_ROLE_ID,
    SPAM_MSG_LIMIT, SPAM_WINDOW_SEC, SPAM_TIMEOUT_SEC,
    POO_CHANNEL_ID,
)
...
# ═══════════════════════════════════════════════════════════════
#   ANTI-SPAM
#   Prati poruke po korisniku u pokretnom prozoru SPAM_WINDOW_SEC.
#   Ako pređe SPAM_MSG_LIMIT — timeout + brisanje poruka.
# ═══════════════════════════════════════════════════════════════
_spam_tracker: dict[int, deque] = defaultdict(deque)  # user_id → deque of timestamps
_spam_warned:  set[int]         = set()               # user_ids koji su već upozoreni


async def _handle_spam(message: discord.Message) -> bool:
    """Vrati True ako je poruka spam (i obriši je), False ako je OK."""
    ...
    if len(dq) >= SPAM_MSG_LIMIT:
        ...
        # Timeout (bešumni fail ako bot nema dozvolu)
        try:
        ...
        # Upozorenje u kanal (samo jednom po korisniku u toku timeoutа)
        if uid not in _spam_warned:
            ...
            try:
                warn_embed = discord.Embed(
                    ...
                    description=(
                        f"{member.mention} — previše poruka u kratkom vremenu!\n"
                        f"Timeout: **{SPAM_TIMEOUT_SEC}s** ⏱️"
                    ),
                    color=0xE74C3C,
                    timestamp=datetime.now(timezone.utc),
                )
                warn_embed.set_footer(text="GIANNI Anti-Spam sistem")
                msg = await message.channel.send(embed=warn_embed, delete_after=8)
            except Exception:
                pass
            # Makni iz warneda nakon timeoutа
...
async def on_member_join(member: discord.Member):
    ...
    if not chan:
        # Fallback — pronađi kanal sa "welcome" u imenu
...
async def on_message(message: discord.Message):
    ...
    # ── .poo prefix → redirect u poo kanal ─────────────────────
